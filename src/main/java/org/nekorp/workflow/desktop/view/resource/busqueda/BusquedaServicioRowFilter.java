 ...
 * @author Nekorp
 */
public class BusquedaServicioRowFilter extends RowFilter {

    private List<Matcher> matchers;
    private boolean acceptAll;
    
    public BusquedaServicioRowFilter(String filtro) {
        if (filtro == null) {
    ...
    public boolean include(Entry entry) {
        ...
        for (Matcher x: matchers) {
            ...
            if (!match) {
                String statusServicio = entry.getStringValue(1).toLowerCase();
                match = x.reset(statusServicio).find();
            }
            if (!match) {
                String statusCobranza = entry.getStringValue(7).toLowerCase();
                match = x.reset(statusCobranza).find();
            }
            if (!match) {
                String nombreCliente = StringNormalizer.normalize(entry.getStringValue(2));
                match = x.reset(nombreCliente).find();
            }
            if (!match) {
                String tipo = entry.getStringValue(5).toLowerCase();
                match = x.reset(tipo).find();
            }
            if (!match) {
                String placas = entry.getStringValue(3).toLowerCase();
                match = x.reset(placas).find();
            }
            if (!match) {
                String desc = StringNormalizer.normalize(entry.getStringValue(6));
                match = x.reset(desc).find();
            }
            if (!match) {
                return false;
            }
