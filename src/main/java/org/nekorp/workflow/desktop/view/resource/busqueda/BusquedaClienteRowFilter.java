 ...
 * @author Nekorp
 */
public class BusquedaClienteRowFilter extends RowFilter {

    private List<Matcher> matchers;
    private boolean acceptAll;
    
    public BusquedaClienteRowFilter(String filtro) {
        if (filtro == null) {
    ...
    public boolean include(Entry entry) {
        ...
        for (Matcher x: matchers) {
            ...
            if (!match) {
                String rfc = entry.getStringValue(2).toLowerCase();
                match = x.reset(rfc).find();
            }
            if (!match) {
                String nombreCliente = StringNormalizer.normalize(entry.getStringValue(1));
